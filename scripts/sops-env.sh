...
#   . scripts/sops-env.sh
# The reader role (24h) is enough here; only openbao-ci-auth.sh and kv patches need role=admin (1h).
...
bao token lookup >/dev/null 2>&1 || { echo "!! no OpenBao session: systemctl --user start tsh-openbao; bao login -method=oidc role=reader (24h, read-only; role=admin for writes, 1h)" >&2; return 1 2>/dev/null || exit 1; }
