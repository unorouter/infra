...
REPO="${1:?usage: build-local.sh <unorouter|new-api|unorouter-bot|uno-import|new-api-sync> [--deploy]}"
DEPLOY="${2:-}"
case "$REPO" in
  unorouter|new-api|unorouter-bot|uno-import|new-api-sync) ;;
  *) echo "unknown repo: $REPO (expected unorouter, new-api, unorouter-bot, uno-import or new-api-sync)" >&2; exit 1 ;;
esac
ROOT="$(cd .. && pwd)"
SRC="$ROOT/$REPO"
...
# operator's own OIDC token (~/.bao-token from `bao login -method=oidc role=reader`, 24h read-only), so the
