...
# Full runbook context: docs/dr.md
set -euo pipefail
cd "$(dirname "$0")/.."
...
SOPS_KEYS() { sops -d secrets/break-glass.sops.yaml | awk '/^unseal_keys:/{f=1;next} /^[^ ]/{f=0} f' | grep -oP '^\s*-\s*\K\S+'; }
...
# Nodes only: the buckets share the state and are prevent_destroy, an untargeted destroy refuses.
destroy() { (cd tofu && set -a && . ./.env && set +a && tofu init -input=false >/dev/null && tofu destroy -target=hcloud_server.node -target=hcloud_firewall.node); }
...
# talos/talsecret.sops.yaml (break-glass age key); the Hetzner console is the last resort.
...
"; }
TALOSCONFIG_FILE=talos/clusterconfig/talosconfig
# Renders clusterconfig/ (machine configs and the talosconfig). talhelper decrypts
# talos/talsecret.sops.yaml and talos/talenv.sops.yaml itself: both open with the break-glass
# age key, so SOPS_AGE_KEY_FILE must point at it (the VeraCrypt volume).
talosconfig() {
  [ -n "${SOPS_AGE_KEY_FILE:-}" ] || { echo "!! SOPS_AGE_KEY_FILE=/run/media/veracrypt1/unorouter/sops-age-keys.txt" >&2; exit 1; }
  (cd talos && talhelper genconfig --no-gitignore >/dev/null)
  echo "$TALOSCONFIG_FILE rendered (clusterconfig/ is gitignored; shred it when done)"
}
kubeconfig() {
  ...
  # the rendered kubeconfig names the cluster endpoint on the WireGuard mesh; point it at this node's tailnet address
  sed -i "s#https://10\.200\.0\.[0-9]*:6443#https://$ip:6443#" kubeconfig.breakglass
...
restore() {
  ...
  # does not exist yet at this point, so the bucket credential comes from secrets/k8s.sops.yaml
  # (break-glass age key, the same pair the writers use and, since 2026-09-15, the pair in
  # tofu/.env); sops rather than .env so a laptop without .env can still restore.
  echo ">> pull latest snapshot from Hetzner (secrets/k8s.sops.yaml newapi-pg-s3 credential)"
  local ak sk
  read -r ak sk < <(sops -d secrets/k8s.sops.yaml | python3 -c 'import sys,yaml; d=[x for x in yaml.safe_load_all(sys.stdin) if x and x["metadata"]["name"]=="newapi-pg-s3"][0]["stringData"]; print(d["ACCESS_KEY_ID"], d["ACCESS_SECRET_KEY"])')
...
cmd="${1:?usage: dr.sh <ips|apply|destroy|bootstrap|talosconfig|kubeconfig|unseal|restore|root>}"
shift
root() { generate_root; }
bao-read() { bao_read "$@"; }
"$cmd" "$@"
